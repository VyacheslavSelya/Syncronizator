...
namespace MoskvinWorkers
{
    public class Files
    {
        ...
        public void AddPath(int fileId, string path)
        {
            if (fileId == -1)
            {
                MessageBox.Show("Выделите строку с источником.", "Нет файла для удаления", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
        ...
        /// <summary>
        /// Очистить список путей источника.
        /// </summary>
        /// <param name="fileId">Номер источника.</param>
        public void ClearPathList(int fileId)
        {
            try
            {
                FileList[fileId].PathToCopy.Clear();
                MessageBox.Show("Список путей успешно очищен!", "Успешно", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Не удалось очистить список путей. Причина:\n" + ex.Message, 
                    "Не удалось", 
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        /// </summary>
        /// <param name="list">List of selected rows in dataGrid.</param>
        public void RemoveFileList(System.Collections.IList list)
        {
            if (list.Count <= 0)
            {
            ...
            else
            {
                ...
                {
                    //FileList.RemoveAt(fileID);
                    FileList.RemoveAt(file.FileID - 1);//RemoveFile(file.FileID - 1); // Удаляем файл.
                }
